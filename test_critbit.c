...
static void test_foreach(CuTest * tc) {
  ...
  counter = 0;
  result = critbit_foreach(&cb, "herpderp", strlen("herpderp"), count_cb, &counter);
  CuAssertIntEquals(tc, 0, result);
  ...
  counter = 0;
  result = critbit_foreach(&cb, "", 0, count_cb, &counter);
  CuAssertIntEquals(tc, 0, result);
  ...
  counter = 0;
  result = critbit_foreach(&cb, "herp", 0, count_cb, &counter);
  CuAssertIntEquals(tc, 0, result);
  CuAssertIntEquals(tc, 4, counter);

  /* keys should be processed in order */
  prev = 0;
  result = critbit_foreach(&cb, "", 0, ordered_cb, (void *)&prev);
  CuAssertIntEquals(tc, 0, result);
}
...
static void test_keyvalue(CuTest * tc) {
  ...
  len = critbit_new_kv(key, len, &i, sizeof(int), buffer);
  CuAssertIntEquals(tc, strlen(buffer) + 1 + sizeof(int), len);
  CuAssertStrEquals(tc, buffer, key);
  critbit_get_kv(buffer, &result, sizeof(int));
  CuAssertIntEquals(tc, i, result);

  result = critbit_insert(&cb, buffer, len);
  CuAssertIntEquals(tc, CB_SUCCESS, result);
  result = critbit_find_prefix(&cb, key, strlen(key) + 1, matches, 2, 0);
  CuAssertIntEquals(tc, 1, result);
  critbit_get_kv(matches[0], &result, sizeof(int));
  CuAssertIntEquals(tc, i, result);
}
